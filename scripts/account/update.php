<?php 
    ...
    if (!move_uploaded_file($image['tmp_name'],constant('ROOTPATH').'\\'.$relPath)) {
        ...
        $result = $repo->editUser($id, $login, $email, $password, $path );
    
        if ($result) {
            echo json_encode(array("login" => $result->getLogin(), "email" => $result->getEmail(), 
                "password" => $result->getPassword(), "imageSrc" => $result->getImageSrc(), 
                "id" => $result->getId()));
        }
        else { 
            // delete from server already loaded file because user updation failed
